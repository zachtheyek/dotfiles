# TODO: understand wtf is happening in this script
#!/bin/bash
...
get_attendees() {
    attendees=$(
        icalBuddy \
            -cf ~/.config/icalBuddy/config.plist \
            ...
            --bullet "" \
            \
            eventsToday # --excludeCals "training,omerxx@gmail.com" \
    )
}
...
parse_attendees() {
    ...
    number_of_attendees=$((${#attendees_array[@]} - 3)) # -3 is a hardcoded adjustment (likely to ignore header or metadata lines) to approximate the number of actual attendees
}
